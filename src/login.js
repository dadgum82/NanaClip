...
async function main() {
  nanaSection('Starting Login Helper');
  ...
  try {
    ...
    if (err.message?.toLowerCase().includes('timeout')) {
      nanaWarn('Oh my, I waited and waited but never heard back from you!');
      nanaWarn('Did you get lost in the checkout lane? Run the script again when ready.');
    } else {
      nanaError(`Something went sideways:  ${err.message}`);
    }
    process.exit(1);
  } finally {
    await browser.close();
...
main().catch(err => {
  nanaError(`Oh goodness:  ${err.message}`);
  process.exit(1);
});
